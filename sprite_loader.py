...
class SpriteLoader:
    ...
    TILESHEET_RECTS = {
        "brick_wall": pygame.Rect(28, 321, 193, 194),
        "slot_machine": pygame.Rect(271, 303, 126, 212),
        "green_rug": pygame.Rect(435, 323, 193, 192),
        "poker_table": pygame.Rect(649, 355, 188, 158),
        "crates": pygame.Rect(859, 329, 144, 185),
        "bar": pygame.Rect(1019, 321, 223, 194),
    }
    SOLID_TILESHEET_OBJECTS = {"brick_wall", "green_rug"}

    def __init__(self, asset_root: str = "Assets", tile_size: int = 64):
        ...
        self._object_cache: dict[str, pygame.Surface] | None = None

    def get_tile(self, name: str) -> pygame.Surface:
        if name not in self.TILE_FILES:
            raise KeyError(f"Unknown tile asset '{name}'")

        cache_key = (name, self.tile_size)
        if cache_key not in self._scaled_cache:
            raw = self._load_image(name, self.asset_root / self.TILE_FILES[name])
            scaled = pygame.transform.scale(raw, (self.tile_size, self.tile_size))
    ...
    def load_tilesheet_objects(self) -> dict[str, pygame.Surface]:
        if self._object_cache is not None:
            return self._object_cache

        sheet = self._load_image("tiles_sheet", self.asset_root / "tiles.png")
        objects: dict[str, pygame.Surface] = {}
        for name, rect in self.TILESHEET_RECTS.items():
            surface = sheet.subsurface(rect).copy()
            if name in self.SOLID_TILESHEET_OBJECTS:
                objects[name] = surface
            else:
                objects[name] = self._prepare_sprite_surface(surface)
    ...
    def _load_image(self, name: str, path: Path) -> pygame.Surface:
        if name not in self._raw_cache:
            ...
            try:
                ...
                # Some pygame builds only decode BMP directly. Pillow keeps
                # PNG/JPEG asset loading working across interpreter setups.
